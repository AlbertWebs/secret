...
$('#contactform').submit(function() {
    ...
    $("#message-contact").slideUp(750, function() {
		...
		if(verify_contact == correct_answer){
    ...
    $("#message-contact").slideUp(750, function() {
		...
		if(verify_contact == correct_answer){
			...
			$.post(action, {
					...
					verify_contact: $('#verify_contact').val(),
					country: $('#country').val(),
					qtyInput_adult: $('#qtyInput_adult').val(),
					qtyInput_12: $('#qtyInput_12').val(),
					qtyInput_child: $('#qtyInput_child').val(),
					dates: $('#dates').val(),
					duration: $('#duration').val(),
					budget: $('#budget').val(),
					destination: $('#destination').val(),
					interest: $('#interest').val(),
					under_12: $('#under_12').val(),
					children: $('#children').val(),
					adult: $('#adult').val(),
					
					prefered_destinations: $('#prefered_destinations').val(),
					
				},
				function(data) {
					document.getElementById('message-contact').innerHTML = data;
					$('#message-contact').slideDown('slow');
					$('#planform .loader').fadeOut('slow', function() { $(this).remove() });
					$('#planform')[0].reset();
					$('#submit-plan').removeAttr('disabled');
					if (data.match('success') != null) $('#planform').slideUp('slow');
